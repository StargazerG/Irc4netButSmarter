...
namespace StargazerG.Irc4NetButSmarter
{
    internal class PrimaryOrFallbackEncoding : Encoding
    {
        ...
        public PrimaryOrFallbackEncoding(Encoding primary, Encoding fallback)
        {
            try
            {
                PrimaryEncoding = GetEncoding(primary.WebName, new EncoderExceptionFallback(), new DecoderExceptionFallback());
            }
            catch (ArgumentException)
            {
                ...
                {
                    throw new ArgumentException("a custom primary encoding's encoder fallback must be an EncoderExceptionFallback");
                }
                if (!(primary.DecoderFallback is DecoderExceptionFallback))
                {
                    throw new ArgumentException("a custom primary encoding's decoder fallback must be a DecoderExceptionFallback");
                }
            }

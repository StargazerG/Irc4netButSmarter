...
namespace StargazerG.Irc4NetButSmarter
{
    ...
    public class ChannelModeMap : Dictionary<char, ChannelModeInfo>
    {
        ...
        public ChannelModeMap(string channelModes) => Parse(channelModes);

        public void Parse(string channelModes)
        {
            string[] modeSplit = channelModes.Split(',');
            string listAlways = modeSplit[0];
            string settingAlways = modeSplit[1];
            string onlySet = modeSplit[2];
            string never = modeSplit[3];
    ...
    public class ChannelModeChangeInfo
    {
        ...
        public static List<ChannelModeChangeInfo> Parse(ChannelModeMap modeMap, string target, string mode, string modeParameters)
        {
            if (modeMap == null)
            {
                throw new ArgumentNullException(nameof(modeMap));
            }
            if (target == null)
            {
